    ...
    <div className="py-4" >
      <div className="flex flex-col md:flex-row relative" >
          ...
          <div className=" w-full">
            {
              pastTrips.length === 0 ? <p className='flex justify-center items-center text-3xl text-black h-[75vh] '>No trips for now</p> :
              <div className="space-y-4 grid sm:grid-cols-2 grid-cols-1 md:grid-cols-3 gap-5">
                {pastTrips.map((trip)=><TripComponent key={trip._id} trips={trip} />)}
              </div> 
            }             
          </div>
      </div>
